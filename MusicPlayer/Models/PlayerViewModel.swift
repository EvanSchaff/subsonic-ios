...
class PlayerViewModel: ObservableObject {
    ...
    private let maxPreloadedPlayers = 2  // Preload next 2 tracks
    ...
    private func generateShuffledIndices() {
        ...
        // Find the current song's position in the shuffled array
        if let currentIndex = shuffledIndices.firstIndex(of: playlist.currentIndex) {
            // Move it to the beginning
            shuffledIndices.swapAt(0, currentIndex)
        }
        
        // Important: Update the playlist's current index to match the first shuffled index
