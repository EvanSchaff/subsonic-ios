...
class SubsonicClient {
    ...
    // Get authentication query string for cover art URLs
    ...
    func getAlbums(type: AlbumListType = .alphabeticalByName, size: Int? = nil) async throws -> [Album] {
        // Build params dictionary conditionally based on whether size is provided
    ...
    func fetchAlbumArt(for albums: [Album]) async -> [Album] {
        ...
        // First check cache and update albums with cached images
...
// MD5 Extension for password hashing
extension String {
    var md5: String {
